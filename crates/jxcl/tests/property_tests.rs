...
fn sample_operands(format: Format, rng: &mut Xorshift64) -> Operands {
    match format {
        ...
        Format::RImm64 => Operands::RImm64 {
            rd: sample_reg(rng),
            imm: rng.next_u64(),
        },
        Format::RMem => Operands::RMem {
            rd: sample_reg(rng),
            base: sample_reg(rng),
            disp: rng.next_u64() as i32,
        },
        Format::MemR => Operands::MemR {
            base: sample_reg(rng),
            disp: rng.next_u64() as i32,
            rs: sample_reg(rng),
        },
        ...
        Format::Imm16 => Operands::Imm16 {
            imm: (rng.next_u64() & 0xFFFF) as u16,
        },
    }
}
    ...
    for def in all_defs() {
        for _ in 0..TRIALS_PER_OPCODE {
            ...
            encode(&instr, &mut bytes)
                .unwrap_or_else(|e| panic!("encode failed for {}: {}", def.mnemonic.text(), e));
            assert_eq!(
                bytes.len(),
                def.format.len(),
