...
fn golden_vectors_match_expected_final_state() {
    for v in VECTORS {
        ...
        match (result, v.expect_fault) {
            ...
            (_, Some(expected)) => panic!(
                "[{}] expected fault {} but got {:?}",
                v.name, expected, result
            ),
            (_, None) => {}
        }
        for (reg, expected) in v.expect_registers {
            ...
            assert_eq!(
                actual, *expected,
                "[{}] R{} mismatch: expected {:#x}, got {:#x}",
                v.name, reg, expected, actual
            );
        }
        if let Some(expected_flags) = v.expect_flags {
            assert_eq!(
                state.registers.flags, expected_flags,
                "[{}] flags mismatch",
                v.name
            );
        }
