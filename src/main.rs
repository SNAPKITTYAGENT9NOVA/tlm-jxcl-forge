...
fn cmd_run(args: &[String]) -> ExitCode {
    ...
    if let Err(e) = state.memory.loader_write(program.header.code_size, program.data) {
    ...
    loop {
        if steps >= limit {
            eprintln!("error: execution limit of {} steps reached without halting", limit);
        ...
        if trace {
            ...
            steps += 1;
            match entry.result {
                jxcl::execution::StepResult::Continued | jxcl::execution::StepResult::Signaled(_) => continue,
