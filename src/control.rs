...
pub fn condition_holds(mnemonic: Mnemonic, flags: Flags) -> bool {
    match mnemonic {
        ...
        other => panic!("condition_holds called with non-conditional mnemonic {:?}", other),
    }
}
...
mod tests {
    ...
    fn signed_less_than_uses_n_xor_v() {
        let f_lt = Flags { z: false, n: true, c: false, v: false };
        assert!(condition_holds(Mnemonic::Jl, f_lt));
        let f_ge = Flags { z: false, n: false, c: false, v: false };
