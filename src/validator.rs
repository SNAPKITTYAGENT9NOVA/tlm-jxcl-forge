...
fn err(kind: ValidationErrorKind, offset: u64, reason: impl Into<String>) -> ValidationError {
    ValidationError { kind, offset, reason: reason.into() }
}
...
pub fn validate(
    ...
    let instrs = decode_all(program.code).map_err(|e| {
        let kind = match e.kind {
            ...
            crate::errors::DecodeErrorKind::InvalidRegister => ValidationErrorKind::BadRegisterOperand,
...
mod tests {
    ...
    fn rejects_branch_into_middle_of_another_instruction() {
        ...
        assert_eq!(result.unwrap_err().kind, ValidationErrorKind::BadBranchTarget);
    }
    ...
    fn rejects_bad_entry_point() {
        ...
        assert_eq!(validate(&bin).unwrap_err().kind, ValidationErrorKind::BadEntryPoint);
    }
    ...
    fn rejects_invalid_opcode() {
        ...
        let bin = binary::write(0, &code, &[]);
        assert_eq!(validate(&bin).unwrap_err().kind, ValidationErrorKind::BadOpcode);
    }
}
