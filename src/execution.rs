...
fn reg_read(state: &MachineState, id: RegId) -> u64 {
    ...
    state.registers.read(id).expect("decoder already validated register operands")
}

fn reg_write(state: &mut MachineState, id: RegId, value: u64) {
    state.registers.write(id, value).expect("decoder already validated register operands");
}
...
pub fn step(state: &mut MachineState) -> StepResult {
    ...
    let instr = match crate::encoding::decoder::decode_one(bytes, pc) {
        ...
        Err(e) => {
            let f = match e.kind {
                ...
                crate::errors::DecodeErrorKind::TruncatedInstruction => ExecutionFault::IllegalOperand,
            };
    ...
    match (instr.mnemonic, instr.operands) {
        ...
        (Mnemonic::Pop, Operands::R { rd }) => {
            let sp = state.registers.sp;
            if sp.checked_add(8).map(|e| e > state.memory.len()).unwrap_or(true) {
                return fault(state, ExecutionFault::StackFault);
            }
        ...
        // ---- Memory / atomic ----
        (Mnemonic::Cas, Operands::Cas { rd, base, rs_new, disp }) => {
            let addr = addr_of(reg_read(state, base), disp);
            ...
            let flags = Flags { z: success, n: (final_rd as i64) < 0, c: false, v: false };
...
fn apply_flags(state: &mut MachineState, def: &crate::isa::opcodes::InstructionDef, computed: Flags) {
