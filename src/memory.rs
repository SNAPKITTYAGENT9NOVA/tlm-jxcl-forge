...
impl Memory {
    ...
    pub fn new(size: u64) -> Self {
        Memory { data: vec![0u8; size as usize], code_start: 0, code_end: 0 }
    }
    ...
    /// (not execute).
    pub fn with_code_region(size: u64, code_start: u64, code_len: u64) -> Result<Self, MemoryFault> {
        let code_end = code_start.checked_add(code_len).ok_or(MemoryFault::InvalidAddress)?;
        if code_end > size {
            return Err(MemoryFault::InvalidAddress);
        }
        Ok(Memory { data: vec![0u8; size as usize], code_start, code_end })
    }
    ...
    pub fn read16(&self, addr: u64) -> Result<u16, MemoryFault> {
        Ok(u16::from_le_bytes(self.read_checked(addr, 2)?.try_into().unwrap()))
    }
    pub fn read32(&self, addr: u64) -> Result<u32, MemoryFault> {
        Ok(u32::from_le_bytes(self.read_checked(addr, 4)?.try_into().unwrap()))
    }
    pub fn read64(&self, addr: u64) -> Result<u64, MemoryFault> {
        Ok(u64::from_le_bytes(self.read_checked(addr, 8)?.try_into().unwrap()))
    }
    ...
    pub fn write16(&mut self, addr: u64, value: u16) -> Result<(), MemoryFault> {
        self.write_checked(addr, 2)?.copy_from_slice(&value.to_le_bytes());
        Ok(())
    }
    pub fn write32(&mut self, addr: u64, value: u32) -> Result<(), MemoryFault> {
        self.write_checked(addr, 4)?.copy_from_slice(&value.to_le_bytes());
        Ok(())
    }
    pub fn write64(&mut self, addr: u64, value: u64) -> Result<(), MemoryFault> {
        self.write_checked(addr, 8)?.copy_from_slice(&value.to_le_bytes());
        Ok(())
    }
