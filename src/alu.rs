...
pub fn add(a: u64, b: u64) -> AluResult {
    ...
    let (z, n) = zn(value);
    AluResult { value, flags: Flags { z, n, c: carry, v } }
}
...
pub fn adc(a: u64, b: u64, carry_in: bool) -> AluResult {
    ...
    let (z, n) = zn(sum2);
    AluResult { value: sum2, flags: Flags { z, n, c: c1 || c2, v: v1 || v2 } }
}
...
pub fn sub(a: u64, b: u64) -> AluResult {
    ...
    let (z, n) = zn(value);
    AluResult { value, flags: Flags { z, n, c: borrow, v } }
}

pub fn sbc(a: u64, b: u64, borrow_in: bool) -> AluResult {
    let full = (a as u128).wrapping_sub(b as u128).wrapping_sub(borrow_in as u128);
    ...
    let (z, n) = zn(value);
    AluResult { value, flags: Flags { z, n, c: borrow, v: v1 || v2 } }
}
...
pub fn inc(a: u64) -> AluResult {
    ...
    let (z, n) = zn(value);
    AluResult { value, flags: Flags { z, n, c: false, v } }
}
...
pub fn mul(a: u64, b: u64) -> AluResult {
    ...
    let (z, n) = zn(value);
    AluResult { value, flags: Flags { z, n, c: overflow, v: overflow } }
}
...
pub fn mulh(a: u64, b: u64) -> AluResult {
    ...
    let (z, n) = zn(value);
    AluResult { value, flags: Flags { z, n, c: false, v: false } }
}
...
pub fn shl(a: u64, count: u64) -> AluResult {
    ...
    let carry = if amt == 0 { false } else { (a >> (64 - amt)) & 1 != 0 };
    let (z, n) = zn(value);
    AluResult { value, flags: Flags { z, n, c: carry, v: false } }
}

pub fn shr(a: u64, count: u64) -> AluResult {
    let amt = shift_amount(count);
    let value = if amt == 0 { a } else { a.wrapping_shr(amt) };
    let carry = if amt == 0 { false } else { (a >> (amt - 1)) & 1 != 0 };
    let (z, n) = zn(value);
    AluResult { value, flags: Flags { z, n, c: carry, v: false } }
}
...
pub fn sar(a: u64, count: u64) -> AluResult {
    let amt = shift_amount(count);
    let signed = a as i64;
    let value = if amt == 0 { signed } else { signed.wrapping_shr(amt) } as u64;
    let carry = if amt == 0 { false } else { (a >> (amt - 1)) & 1 != 0 };
    let (z, n) = zn(value);
    AluResult { value, flags: Flags { z, n, c: carry, v: false } }
}
