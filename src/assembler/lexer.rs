...
fn parse_number(text: &str, line: usize) -> Result<i128, AssemblerError> {
    let bad = |reason: String| AssemblerError { kind: AssemblerErrorKind::MalformedImmediate, line, reason };
    if let Some(hex) = text.strip_prefix("0x").or_else(|| text.strip_prefix("0X")) {
        i128::from_str_radix(hex, 16).map_err(|e| bad(format!("invalid hex literal {:?}: {}", text, e)))
    } else if let Some(bin) = text.strip_prefix("0b").or_else(|| text.strip_prefix("0B")) {
        i128::from_str_radix(bin, 2).map_err(|e| bad(format!("invalid binary literal {:?}: {}", text, e)))
    } else {
        text.parse::<i128>().map_err(|e| bad(format!("invalid decimal literal {:?}: {}", text, e)))
    }
}
...
mod tests {
    ...
    fn tokenizes_memory_operand() {
        ...
        assert_eq!(
            toks,
            vec![Token::LBracket, Token::Ident("R2".into()), Token::Plus, Token::Number(8), Token::RBracket]
        );
    }
