...
fn err(kind: BinaryFormatErrorKind, reason: impl Into<String>) -> BinaryFormatError {
    BinaryFormatError { kind, reason: reason.into() }
}
...
pub fn parse(bytes: &[u8]) -> Result<Program<'_>, BinaryFormatError> {
    if bytes.len() < HEADER_SIZE {
        return Err(err(BinaryFormatErrorKind::TooShort, "file shorter than the 56-byte header"));
    }
    ...
    if architecture != ARCHITECTURE_ID {
        return Err(err(
            ...
            format!("expected architecture {:#06x}, found {:#06x}", ARCHITECTURE_ID, architecture),
        ));
    }
    ...
    if reserved != 0 {
        return Err(err(BinaryFormatErrorKind::BadMagic, "reserved header field must be zero"));
    }

    let code_end = code_offset
        .checked_add(code_size)
        .ok_or_else(|| err(BinaryFormatErrorKind::SizeOutOfRange, "code section overflows u64"))?;
    if code_end > bytes.len() as u64 {
        return Err(err(BinaryFormatErrorKind::OffsetOutOfRange, "code section exceeds file length"));
    }
    let data_end = data_offset
        .checked_add(data_size)
        .ok_or_else(|| err(BinaryFormatErrorKind::SizeOutOfRange, "data section overflows u64"))?;
    if data_end > bytes.len() as u64 {
        return Err(err(BinaryFormatErrorKind::OffsetOutOfRange, "data section exceeds file length"));
    }
...
mod tests {
    ...
    fn rejects_bad_magic() {
        ...
        bin[0] = b'X';
        assert_eq!(parse(&bin).unwrap_err().kind, BinaryFormatErrorKind::BadMagic);
    }

    #[test]
    fn rejects_truncated_header() {
        let bin = vec![0u8; 10];
        assert_eq!(parse(&bin).unwrap_err().kind, BinaryFormatErrorKind::TooShort);
    }
    ...
    fn rejects_oversized_code_section() {
        ...
        assert_eq!(parse(&bin).unwrap_err().kind, BinaryFormatErrorKind::OffsetOutOfRange);
    }
}
